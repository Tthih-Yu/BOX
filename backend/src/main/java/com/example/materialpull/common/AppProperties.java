...
public class AppProperties {
    ...
    /**
     * 打印下发模式。true=拉模式（本地打印代理轮询领取，适用于USB打印机场景，推荐）；
     * false=推模式（后端主动HTTP推送到 printSubmitUrl）。
     */
    private boolean printPullMode = true;
    /**
     * 拉模式下，已被代理领取（SENT）但超过该秒数仍未回传结果的打印作业，
     * 会在下次领取时被重新分配，避免代理崩溃导致标签永远打不出来。
     */
    private long printClaimStaleSeconds = 120;
