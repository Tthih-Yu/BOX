        ...
        <div class="login-card">
          <header class="login-card__header">
            <div class="login-card__icon">
              <el-icon><component :is="loginMode==='admin' ? 'Setting' : 'UserFilled'" /></el-icon>
            </div>
            <h2>欢迎登录</h2>
          </header>
...
import { resetMenuWhitelist } from '../permissions'
...
async function login() {
  ...
  try {
    ...
    // 管理员入口只放行 ADMIN；防止管理员账号从用户入口登录后仍受菜单收窄限制。
    if (loginMode.value === 'admin' && r.role !== 'ADMIN') {
      ElMessage.error('该账号不是管理员，请从“用户登录”入口登录')
      return
    }
    if (loginMode.value === 'user' && r.role === 'ADMIN') {
      ElMessage.warning('管理员账号建议从“管理员登录”入口登录')
    }
