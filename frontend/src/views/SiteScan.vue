<template>
  <el-row :gutter="18">
    <el-col :span="10">
      <div class="card">
        <h3>现场扫码工作台</h3>
        <p class="hint">支持三类现场动作：盒子用完拉动、现场收货确认、异常上报。工位二维码扫出物料号(+工位)，系统按物料号查料号映射、按工位锁定仓库代号；同物料同工位的 A/B 两个仓库代号会各生成一条任务、各打一张仓库标签。两个盒子扫哪个都一样。</p>
        ...
        <el-form label-width="110px">
          <el-form-item label="扫码内容"><el-input v-model="form.scanCode" clearable placeholder="工位二维码内容：物料号 或 物料号|工位地址" autofocus /></el-form-item>
          <el-form-item v-if="mode==='empty'" label="工位地址"><el-input v-model="form.stationCode" clearable placeholder="可选；二维码已含工位则自动带出，同物料多工位时必填" /></el-form-item>
    ...
    <el-col :span="14">
      <div class="card">
        ...
        <div v-if="result">
          ...
          <div v-if="result?.generatedTasks?.length" style="margin-top:12px">
            <h4>本次生成的仓库任务（每个仓库代号一张标签）</h4>
            <el-table :data="result.generatedTasks" size="small" border>
              <el-table-column prop="warehouseCode" label="仓库代号(条形码)" />
              ...
              <el-table-column prop="sendStationAddress" label="发送工位地址" />
              <el-table-column label="状态" width="110">
                <template #default="{ row }">
                  <el-tag v-if="row.duplicateBlocked" type="info">已存在,跳过</el-tag>
                  <el-tag v-else :type="tagType(row.taskStatus)">{{ row.taskStatus }}</el-tag>
                </template>
              </el-table-column>
            </el-table>
          </div>
...
const form = reactive<any>({ scanCode:'', stationCode:'', taskNo:'', emptyContainerNo:'', exceptionType:'', reason:'', deviceNo:runtimeDeviceNo(), allowRepeat:false })
...
onMounted(async()=>{ const meta = await loadBusinessMeta(); exceptionTypeOptions.value = meta.exceptionTypes; loadCandidates() })
</script>
