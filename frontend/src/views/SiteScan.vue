<template>
  <el-row :gutter="18">
    <el-col :span="10">
      <div class="card">
        <h3>现场扫码工作台</h3>
        <p class="hint">工位二维码内容为「物料号,工位,使用/备用」，例如 13799816,物料架-01-F03,备用。系统自动解析：物料号查料号映射、工位锁定仓库代号、使用→正常配送 / 备用→紧急配送。扫码内容直接整段粘贴即可。</p>
        ...
        <el-form label-width="110px">
          <el-form-item label="扫码内容"><el-input v-model="form.scanCode" clearable placeholder="整段粘贴二维码内容，如 13799816,物料架-01-F03,备用" autofocus /></el-form-item>
          <el-form-item v-if="mode==='empty'" label="工位地址"><el-input v-model="form.stationCode" clearable placeholder="可选；二维码已含工位则自动解析，无需手填" /></el-form-item>
          <el-form-item v-if="mode==='empty'" label="用途">
            <el-radio-group v-model="form.usageType">
              <el-radio label="">自动(按二维码)</el-radio>
              <el-radio label="USE">使用/正常</el-radio>
              <el-radio label="SPARE">备用/紧急</el-radio>
            </el-radio-group>
          </el-form-item>
...
const form = reactive<any>({ scanCode:'', stationCode:'', usageType:'', taskNo:'', emptyContainerNo:'', exceptionType:'', reason:'', deviceNo:runtimeDeviceNo(), allowRepeat:false })
