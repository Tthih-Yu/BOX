...
const STATUS_CN:Record<string,string> = {
  CREATED: '已创建', ACCEPTED: '已接单', PICKING: '拣料中', PICKED: '拣料完成',
  DELIVERING: '配送中', ARRIVED: '已到达', COMPLETED: '已完成', CANCELLED: '已取消', EXCEPTION: '异常'
}
function statusText(v:any){ return v ? (STATUS_CN[String(v).toUpperCase()] || v) : '-' }
const DELIVERY_CN:Record<string,string> = { NORMAL: '正常', URGENT: '紧急', DIRECT: '直送' }
function deliveryModeText(v:any){ return v ? (DELIVERY_CN[String(v).toUpperCase()] || v) : '-' }
    ...
    return () => h('div', { class: 'lane-list' }, [
      props.rows.length ? props.rows.map((row:any) => h('div', {
        ...
        h('div', { class: 'task-main' }, [
          h('b', row.stationCode || row.stationName || '-'),
          h('span', row.materialName || row.materialCode || '-')
        ]),
        h('div', { class: 'task-meta' }, [
          h('span', `数量 ${row.requestQty || 0}`),
          h('span', deliveryModeText(row.deliveryMode)),
          h(ElTag, { type: tagType(row.status) as any, effect: 'dark', size: 'small' }, () => statusText(row.status))
        ])
