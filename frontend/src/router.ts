...
import { canRoute, ensureMenuWhitelist, firstVisibleRoute } from './permissions'
...
const routes: RouteRecordRaw[] = [
  ...
  { path: '/simple-bom', component: () => import('./views/SimpleBom.vue') },
  { path: '/weekly-plan', component: () => import('./views/WeeklyPlan.vue') },
  { path: '/weekly-plan-template', component: () => import('./views/WeeklyPlanTemplate.vue') },
  ...
  { path: '/menu-permissions', component: () => import('./views/MenuPermissions.vue') },
...
router.beforeEach(async (to) => {
  ...
  if (to.path !== '/login') {
    await ensureMenuWhitelist()
    if (!canRoute(to.path)) {
      // 白名单收窄后首页看板可能不可见，回退到第一个可见菜单，避免死循环。
      if (to.path === '/dashboard') { const home = firstVisibleRoute(); if (home && home !== '/dashboard') return home }
      else return firstVisibleRoute() || '/dashboard'
    }
  }
})
