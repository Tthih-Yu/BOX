<template>
  <div class="planning-page">
    <div class="card flow-card">
      <h3>仓库计划链路</h3>
      <div class="flow-line">
        <div v-for="item in flowSteps" :key="item.title" class="flow-node">
          <b>{{ item.title }}</b>
          <span>{{ item.desc }}</span>
        </div>
      </div>
...
const flowSteps = [
  { title: 'SAP / IMS建立链接', desc: 'BOM、料号、库存关联' },
  { title: '工艺仓库录入', desc: 'Bundle、周期、架位、单盒量' },
  { title: 'PPC放入计划', desc: '产量、产线、工位' },
  { title: '生成单盒需求', desc: '一单一盒，按需配料' },
  { title: 'MPC预测买料', desc: '库存扣减、额定值、采购建议' },
]
...
<style scoped>
.planning-page{display:flex;flex-direction:column;gap:16px}.flow-card h3{margin-top:0}.flow-line{display:grid;grid-template-columns:repeat(5,1fr);gap:12px}.flow-node{border:1px solid #dbeafe;background:#f8fbff;border-radius:12px;padding:12px;position:relative}.flow-node:not(:last-child)::after{content:'→';position:absolute;right:-13px;top:32px;color:#409eff;font-weight:700}.flow-node b{display:block;font-size:15px;margin-bottom:6px}.flow-node span{font-size:12px;color:#64748b}.section-title{display:flex;align-items:baseline;gap:12px}.section-title h3{margin-top:0}.section-title span{font-size:12px;color:#64748b}.compact-form :deep(.el-form-item){margin-bottom:10px}
</style>
