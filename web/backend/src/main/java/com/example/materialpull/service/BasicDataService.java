...
public class BasicDataService {
    ...
    public MaterialMappingEntity saveMapping(MaterialMappingEntity entity) {
        ...
        entity.setLineMaterialCode(guard.notBlank(entity.getLineMaterialCode(), "线边料号"));
        entity.setWarehouseMaterialCode(guard.notBlank(entity.getWarehouseMaterialCode(), "仓库料号"));
        mappingRepository.findByLineMaterialCodeAndEnabledTrue(entity.getLineMaterialCode()).ifPresent(old -> {
            if (!Objects.equals(old.getId(), entity.getId()) && Boolean.TRUE.equals(entity.getEnabled())) {
                throw new BusinessException(ErrorCode.DATA_DIRTY, "该线边料号已有启用映射：" + entity.getLineMaterialCode());
            }
        });
    ...
    public StationMaterialEntity saveStationMaterial(StationMaterialEntity s) {
        ...
        if (s.getWarehouseMaterialCode() == null || s.getWarehouseMaterialCode().isBlank()) {
            mappingRepository.findByLineMaterialCodeAndEnabledTrue(s.getMaterialCode())
                    .ifPresent(m -> s.setWarehouseMaterialCode(m.getWarehouseMaterialCode()));
        }
